...
export async function POST(request: Request) {
  ...
  const prompt = [
    ...
    "자연어로 들어온 질문을 그대로 이해해서 한국어로 자연스럽고 간결하게 답한다.",
    "구조화된 분석 카드와 관련 기록만 근거로 답한다.",
    "모르는 내용은 추측하지 말고, 가능성은 추론 또는 해석이라고 분리해서 말한다.",
    "응답은 반드시 '요약', '근거', '패턴', '제안' 네 구획으로 나눠 작성한다.",
  ...
  return NextResponse.json({ answer: answer || "AI 응답을 생성하지 못했습니다." });
}

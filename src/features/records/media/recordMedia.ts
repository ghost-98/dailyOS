...
export type RecordMediaPreview = LifeMediaUploadInput & {
  id: string;
  lastModified: number;
  mimeType: string;
  name: string;
...
export async function createRecordMediaPreview(file: File): Promise<RecordMediaPreview> {
...
export function formatMediaMetaLines(
  media: Pick<RecordMediaPreview, "durationSeconds" | "height" | "lastModified" | "mimeType" | "sizeBytes" | "takenAt" | "width">,
) {
...
export function hasGeoMetadata(media: Pick<LifePhotoRecord, "latitude" | "longitude"> | Pick<RecordMediaPreview, "latitude" | "longitude">) {
...
export function formatGeoMetadata(media: Pick<LifePhotoRecord, "latitude" | "longitude"> | Pick<RecordMediaPreview, "latitude" | "longitude">) {
...
function readImageDimensions(objectUrl: string) {
  return new Promise<Pick<RecordMediaPreview, "height" | "width">>((resolve, reject) => {
...
function readVideoMetadata(objectUrl: string) {
  return new Promise<Pick<RecordMediaPreview, "durationSeconds" | "height" | "width">>((resolve, reject) => {
...
async function readImageExifMetadata(file: File): Promise<Pick<RecordMediaPreview, "latitude" | "longitude" | "takenAt">> {
...
function parseJpegExif(buffer: ArrayBuffer): Pick<RecordMediaPreview, "latitude" | "longitude" | "takenAt"> {
...
function parseExifPayload(view: DataView, tiffStart: number): Pick<RecordMediaPreview, "latitude" | "longitude" | "takenAt"> {

...
import { emptyLifeDataSnapshot, loadLifeDataForMode } from "@/features/life/dataLoader";
...
export function useLifeDataState(activeTab: LifeDataMode) {
  const { data, isLoading, reload, setData } = useAsyncData({
    deps: [activeTab],
    initialData: emptyLifeDataSnapshot,
    load: () => loadLifeDataForMode(activeTab),
  ...
  const createDailyLog = async (date: string, content: string, linkedTarget?: LifeLinkedTarget) => {
    ...
    if (!savedLog) return;
    setData((current) => ({ ...current, dailyLogs: [savedLog, ...current.dailyLogs] }));
  };
  ...
  const deleteDailyLog = async (id: string) => {
    ...
    setData((current) => ({ ...current, dailyLogs: current.dailyLogs.filter((item) => item.id !== id) }));
  };
  ...
  const uploadLifePhotos = async (date: string, uploads: LifeMediaUploadInput[], caption?: string, linkedTarget?: LifeLinkedTarget) => {
    ...
    setData((current) => ({ ...current, lifePhotos: [...savedPhotos, ...current.lifePhotos] }));
  };

  const deleteLifePhoto = async (photo: LifePhotoRecord) => {
    await deleteLifePhotoFromDb(photo);
    setData((current) => ({ ...current, lifePhotos: current.lifePhotos.filter((item) => item.id !== photo.id) }));
  };
  ...
  const deleteSourceFromActivity = async (activity?: LifeActivityRecord) => {
    ...
    if (activity.sourceType === "todo") {
      ...
      setData((current) => ({ ...current, tasks: current.tasks.filter((task) => task.id !== activity.sourceId) }));
      return;
    }

    await deleteCalendarEventFromDb(activity.sourceId);
    setData((current) => ({ ...current, events: current.events.filter((event) => event.id !== activity.sourceId) }));
  };

  const refreshExpenses = async () => {
    const nextExpenses = await fetchExpenseRecordsFromDb();
    setData((current) => ({ ...current, expenses: nextExpenses ?? [] }));
  };

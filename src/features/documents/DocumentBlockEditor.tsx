...
import { ChevronDown, ChevronUp, ImagePlus, Minus, Plus, Trash2 } from "lucide-react";
...
export function DocumentBlockEditor({
  ...
  return (
    ...
    >
      ...
      <DocumentBlockFields block={block} isUploading={isUploading} onAddBelow={(type) => onAddBelow(type, index)} onChange={onChange} onReplaceImage={onReplaceImage} />
    </article>
  );
}
...
function DocumentBlockFields({
  ...
  if (block.type === "divider") {
    return <div className="doc-divider" />;
  }
          ...
          {block.items.map((item) => (
            <ChecklistRow
              ...
              onChange={(nextItem) => onChange({
                ...block,
                items: block.items.map((current) => current.id === nextItem.id ? nextItem : current),
              })}
              onRemove={() => onChange({
                ...block,
                items: block.items.length === 1 ? [createChecklistItem()] : block.items.filter((current) => current.id !== item.id),
              })}
            />
          ))}
        </div>
  ...
  if (block.type === "toggle") {
    return (
      <div className="doc-block__body">
        ...
        {block.isOpen ? <textarea placeholder="열었을 때 보일 내용" rows={4} value={block.body} onChange={(event) => onChange({ ...block, body: event.target.value })} /> : null}
      </div>
    );
  }
    ...
    return (
      <div className="doc-block__body">
        ...
        <div className="doc-table-actions">
          <ActionButton className="doc-inline-action" onClick={() => onChange({ ...block, rows: [...block.rows, new Array(block.rows[0]?.length ?? 2).fill("")] })} variant="secondary">
            <Plus aria-hidden size={14} />
            행 추가
          </ActionButton>
          <ActionButton className="doc-inline-action" onClick={() => onChange({ ...block, rows: block.rows.map((row) => [...row, ""]) })} variant="secondary">
            <Plus aria-hidden size={14} />
            열 추가
          </ActionButton>
          <ActionButton className="doc-inline-action" disabled={block.rows.length <= 1} onClick={() => onChange({ ...block, rows: block.rows.slice(0, -1) })} variant="secondary">
            <Minus aria-hidden size={14} />
            마지막 행 제거
          </ActionButton>
          <ActionButton className="doc-inline-action" disabled={(block.rows[0]?.length ?? 0) <= 1} onClick={() => onChange({ ...block, rows: block.rows.map((row) => row.slice(0, -1)) })} variant="secondary">
            <Minus aria-hidden size={14} />
            마지막 열 제거
          </ActionButton>
        </div>
  ...
  return (
    <div className="doc-block__body">
      <textarea
        placeholder={block.type.startsWith("heading") ? "제목 입력" : block.type === "quote" ? "인용할 문장" : "내용 입력"}
        rows={rows}
        value={block.text}
...
function ChecklistRow({
  ...
  return (
    <div className="doc-checklist__row">
      <label>
        <input checked={item.checked} type="checkbox" onChange={(event) => onChange({ ...item, checked: event.target.checked })} />
      </label>
      <input placeholder="체크할 내용" value={item.text} onChange={(event) => onChange({ ...item, text: event.target.value })} />
      <IconButton label="항목 삭제" onClick={onRemove} size="sm" tone="ghost">
